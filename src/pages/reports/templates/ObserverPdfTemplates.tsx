...
const getParishInfo = (schoolId: string, users: UserData[]) => {
  const schoolUser = users.find(
    (u) => (u.uid === schoolId || u.id === schoolId) && u.role === "school",
  );
...
export const ObserverDirPdfTemplate = ({
  ...
  const grouped = observers.reduce(
    (acc, obs) => {
      const { forane: lookupForane, name: lookupName } = getParishInfo(
        obs.parishId || obs.schoolId || "",
        users,
      );

      const parishName =
        obs.parishName || (obs as any).schoolName || lookupName;
      const foraneName =
        lookupForane === "Other" && parishName.includes("-")
          ? parishName.split("-")[0].trim()
          : lookupForane;
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <span>
            <strong style={{ color: "#334155" }}>Year:</strong>{" "}
            {academicYear || "All"}
          </span>
          <span>
            <strong style={{ color: "#334155" }}>Forane:</strong>{" "}
            {foraneFilter || "All"}
          </span>
          <span>
            <strong style={{ color: "#334155" }}>Parish:</strong>{" "}
            {parishFilter || "All"}
          </span>
          <span>
            <strong style={{ color: "#334155" }}>Date:</strong> {todayDate}
          </span>
        </div>
      </div>
      ...
      >
        ...
        >
          <thead>
            ...
            >
              <th
                style={{ padding: "16px", width: "40px", textAlign: "center" }}
              >
                #
              </th>
                      ...
                      <React.Fragment key={pName}>
                        ...
                        >
                          ...
                          >
                            ...
                            Parish:{" "}
                            <span style={{ color: "#0f172a" }}>{pName}</span>
                          </td>
                        </tr>
...
export const ObserverAssignPdfTemplate = ({
  ...
  const grouped = assignments.reduce(
    (acc, a) => {
      ...
      const sourceId =
        a.sourceSchoolId || t?.parishId || (t as any)?.schoolId || "";
      const { forane: lookupForane, name: lookupName } = getParishInfo(
        sourceId,
        users,
      );
  ...
  return (
    <div
      id="observer-assign-pdf-container"
      style={{
        padding: "50px 60px",
        backgroundColor: "#ffffff",
        width: "800px",
        ...
        >
          <thead>
            ...
            >
              ...
              <th style={{ padding: "16px", width: "25%" }}>Observer Name</th>
              <th style={{ padding: "16px", width: "25%" }}>Assigned To</th>
              <th style={{ padding: "16px", width: "15%" }}>Class</th>
              <th style={{ padding: "16px", width: "15%" }}>Phone</th>
              <th style={{ padding: "16px", width: "15%" }}>Date</th>
            </tr>
          </thead>
          <tbody>
                      ...
                      <React.Fragment key={pName}>
                        ...
                        {grouped[fName][pName].map((assign: any, i: number) => {
                          const t = teachers.find(
                            (teach) => teach.id === assign.teacherId,
                          );
                          const assignedParishInfo = getParishInfo(
                            assign.targetSchoolId || "",
                            users,
                          );
                          const assignedName =
                            assign.targetSchoolName &&
                            assign.targetSchoolName.trim() !== ""
                              ? assign.targetSchoolName
                              : assignedParishInfo.name;
                          return (
                            <tr
                              key={assign.id || i}
                              style={{ borderBottom: "1px solid #f1f5f9" }}
