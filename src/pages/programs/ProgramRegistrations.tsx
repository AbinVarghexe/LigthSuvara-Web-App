...
export function ProgramRegistrations() {
    ...
    // Helpers
    const getSchoolName = (schoolId: string, fallback: string = "Unknown School") => {
    ...
    const schoolGroups = useMemo(() => {
        ...
        const groups: Record<string, { count: number; fallbackName: string }> = {};
        registrations
            ...
            .forEach((reg) => {
                ...
                if (!groups[sid]) {
                    groups[sid] = { count: 0, fallbackName: reg.schoolName || "Unknown School" };
                }
        ...
        return Object.entries(groups)
            ...
            .filter((s) => {
                ...
                // 1. Search Filter
                if (searchSchoolsQuery && !schoolName.toLowerCase().includes(searchSchoolsQuery.toLowerCase())) {
                    return false;
                }
    ...
    }, [registrations, selectedProgramName, users, searchSchoolsQuery, selectedForane, selectedParishId]);
    ...
    const currentSchoolProfile = profileSchoolId ? getSchoolProfile(profileSchoolId) : null;
    ...
    return (
        <div className="space-y-6 max-w-6xl mx-auto pb-10">
            ...
            <div className="flex items-center gap-4">
                {level !== "programs" && (
                    <Button variant="outline" size="icon" onClick={goBack} className="rounded-full">
                ...
                <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                        {level === "programs" && "Overview of students registered across all active programs"}
                        {level === "schools" && "Distribution of registrations per participating school"}
                                        ...
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                ...
                                                <div className="flex items-center gap-1.5 mt-1">
                                                    ...
                                                    <span className="text-sm font-medium text-gray-600">{p.count} Students</span>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <CardContent className="p-6 flex items-center justify-between">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                ...
                                                <div className="flex items-center gap-1.5 mt-1">
                                                    <Users className="h-3.5 w-3.5 text-gray-400" />
                                                    <span className="text-sm font-medium text-gray-600">{s.count} Students</span>
                                                </div>
                                            </div>
                                        </div>
                        ...
                        <Card key={st.id || idx}>
                            <CardContent className="p-5 flex items-center gap-5">
                                <div className="h-12 w-12 rounded-full bg-blue-50 border-2 border-blue-100 flex items-center justify-center flex-shrink-0">
                                    {st.isCountOnly ? (
                                        <span className="font-bold text-blue-700">+{st.studentCount}</span>
                                    ) : (
                                ...
                                <div className="flex-1 min-w-0">
                                    <h3 className="font-bold text-gray-900 truncate">
                                        {st.isCountOnly ? `${st.studentCount} Students (Conolidated)` : st.studentName}
                                    </h3>
                                    <div className="flex flex-wrap gap-x-4 gap-y-1 mt-1">
                                        <div className="flex items-center gap-1.5 text-sm text-gray-500">
                                            {st.isCountOnly ? <School className="h-3.5 w-3.5" /> : <Phone className="h-3.5 w-3.5" />}
                                            <span>{st.isCountOnly ? "Bulk Registration" : (st.studentPhone || "No Phone")}</span>
                                        </div>
                                        <div className="flex items-center gap-1.5 text-sm text-gray-500">
                                            <Calendar className="h-3.5 w-3.5" />
                                            <span>Submitted: {st.submittedAt?.toDate().toLocaleDateString("en-US", { month: "short", day: "numeric" })}</span>
                                        </div>
                                    </div>
                                </div>
                                <div className="flex items-center gap-2">
                                    {st.isCountOnly ? (
                                        <Button variant="ghost" size="icon" onClick={() => openProfile(st.schoolUserId)}>
            ...
            <Dialog open={isProfileOpen} onOpenChange={setIsProfileOpen}>
                <DialogContent className="sm:max-w-[450px]">
                    ...
                    {currentSchoolProfile ? (
                        <div className="space-y-5 py-4">
                            <div className="space-y-1">
                                ...
                                <p className="text-lg font-bold text-gray-900 leading-tight">
                                    {currentSchoolProfile.schoolName || currentSchoolProfile.schoolname || currentSchoolProfile.name}
                                </p>
                            </div>

                            <div className="grid grid-cols-1 gap-4">
                                {[
                                    { icon: User, label: "Contact Person", value: currentSchoolProfile.fullName || "Not Set" },
                                    { icon: Phone, label: "Phone Number", value: currentSchoolProfile.phoneNumber || "Not Set" },
                                    { icon: Mail, label: "Email Address", value: currentSchoolProfile.email || "Not Set" },
                                    { icon: MapPin, label: "Forane / Parish", value: `${currentSchoolProfile.forane || "Unknown"} Forane` }
                                ].map((item, i) => (
                        ...
                        <div className="py-10 text-center text-gray-500">Profile data not found.</div>
                    )}
                    <DialogFooter>
                        <Button onClick={() => setIsProfileOpen(false)} className="w-full bg-blue-600 hover:bg-blue-700">Close</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
        </div>
