...
import { ObserverDutyOrderPdfTemplate, ObserverAdminPdfTemplate, ObserverSubmissionReportPdfTemplate } from '../../../pages/reports/templates/ObserverIndividualPdfTemplates';
...
export const IndividualObserverPdfService = {
    ...
    generateObserverReportPdf: async (assignment: any) => {
        const wrapper = document.createElement('div');
        wrapper.style.position = 'absolute';
        wrapper.style.left = '-9999px';
        wrapper.style.top = '-9999px';
        ...
        try {
            ...
            await new Promise<void>((resolve) => {
                root.render(
                    <div id="observer-submission-pdf-container">
                        <ObserverSubmissionReportPdfTemplate
                            assignment={assignment}
                            logoBase64={logoBase64}
                        />
                    </div>
            ...
            const element = document.getElementById('observer-submission-pdf-container');
            if (!element) throw new Error('PDF container not found');
            ...
            pdf.addImage(imgData, 'JPEG', margin, margin, effectiveWidth, imgHeight);
            pdf.save(`Observer_Submission_Report_${assignment.teacherName.replace(/\s+/g, '_')}.pdf`);

        } catch (error) {
            console.error('Observer Report PDF failure:', error);
            throw error;
        } finally {
            try { document.body.removeChild(wrapper); } catch (e) { /* ignore */ }
        }
