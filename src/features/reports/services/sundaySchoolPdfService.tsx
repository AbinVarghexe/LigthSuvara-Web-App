...
import { SundaySchoolPdfTemplate } from "../../../pages/reports/templates/SundaySchoolPdfTemplate";
// @ts-ignore
import html2pdf from "html2pdf.js";

export const PremiumSundaySchoolPdfService = {
    /**
     * Generates a premium PDF report for Sunday Schools using html2pdf.js.
     */
    generateReport: async (users: UserData[], forane: string, parish: string) => {
        // 1. Render React component to static HTML string
        const htmlString = renderToString(
            <SundaySchoolPdfTemplate
                users={users}
                forane={forane}
                parish={parish}
            />
    ...
    `;
        ...
        try {
            ...
            const opt = {
                margin: 10,
                filename: `Sunday_School_Report_${new Date().toISOString().split('T')[0]}.pdf`,
            ...
            console.error('Premium Sunday School report generation failed:', error);
            throw error;
        } finally {
            // Cleanup
